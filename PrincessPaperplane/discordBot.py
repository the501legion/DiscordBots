...
async def on_ready():
    ...
    for guild in bot.guilds:
        print("- " + guild.name)
        if guild.id == config.TEST_SERVER:
    ...
    await ROLES.update_reaction_msg(config.ROLE_CHANNEL, config.ROLES, config.EMOTES, config.TEXT)
    await ROLES.update_reaction_msg(config.ROLE_CHANNEL_TEST, config.ROLES_TEST, config.EMOTES_TEST, config.TEXT_TEST)
